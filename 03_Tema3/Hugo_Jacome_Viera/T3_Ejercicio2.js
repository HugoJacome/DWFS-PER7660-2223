...
function duplicador(lista) {
    return lista.filter(n => n % 2 == 0).map(n => n * 2);
}
...
function media(lista) {
    return lista.reduce((acc, curr) => acc + curr) / lista.length
}
...
function eliminarDuplicados(lista) {
    return lista.filter((n) => lista.indexOf(n) === lista.lastIndexOf(n));
}
 ...
 * 
 */
function nCharConsec(caracter, numRepetido, cadena) {
    let cantidad = 0;
    let maxRepetido = 0;
    cadena.split("").forEach(x => {
        if (x === caracter) {
            cantidad++;
            maxRepetido = maxRepetido < cantidad ? cantidad : maxRepetido;
        } else{
            cantidad = 0;
        }
    });
    return maxRepetido >= numRepetido;
}
